...
def run_outfit_pipeline(trigger_data):
    ...
    logging.warning("Using legacy run_outfit_pipeline - consider updating to async")
    return asyncio.run(run_enhanced_outfit_pipeline(trigger_data))
    """
    Enhanced pipeline with LLM agents and hierarchical fallback:
    Gemini API -> Groq API -> Logic Engine -> Error
    ...
    try:
        ...
        # Step 2: Prepare LLM-optimized context
        logging.info("Preparing LLM context...")
        try:
            llm_context = wardrobe_data_manager.get_llm_optimized_context(
                desired_aesthetic, weather_tag
            )
        ...
        # Step 3: Try Gemini API (Primary Agent)
        logging.info("🤖 Attempting outfit generation with Gemini API...")
        success, outfit_items, error_msg = outfit_llm_agents.generate_outfit_with_gemini(llm_context)
        
        if success and outfit_items:
            logging.info("✅ Gemini API generated outfit successfully")
            return _finalize_outfit(page_id, outfit_items, "gemini", {
                "weather": {"temp": avg_temp, "condition": condition, "tag": weather_tag},
                "aesthetic": desired_aesthetic,
                "user_prompt": user_prompt,
                "items_count": len(outfit_items)
        ...
        # Step 4: Try Groq API (Secondary Agent)
        logging.info("🤖 Attempting outfit generation with Groq API...")
        success, outfit_items, error_msg = outfit_llm_agents.generate_outfit_with_groq(llm_context)
        
        if success and outfit_items:
            logging.info("✅ Groq API generated outfit successfully")
            return _finalize_outfit(page_id, outfit_items, "groq", {
                "weather": {"temp": avg_temp, "condition": condition, "tag": weather_tag},
                "aesthetic": desired_aesthetic,
                "user_prompt": user_prompt,
                "items_count": len(outfit_items)
        ...
        # Step 5: Fallback to Logic Engine
        logging.info("🔧 Falling back to logic engine...")
        try:
            # Get all suitable items for logic engine
            ...
            outfit_items = build_outfit(wardrobe_items, is_hot, desired_aesthetic, washed_required="Done")
            
            if outfit_items:
                logging.info("✅ Logic engine generated outfit successfully")
                return _finalize_outfit(page_id, outfit_items, "logic", {
                    "weather": {"temp": avg_temp, "condition": condition, "tag": weather_tag},
                    "aesthetic": desired_aesthetic,
                    "user_prompt": user_prompt,
                    "items_count": len(outfit_items)
...
async def _finalize_outfit(page_id: str, outfit_items: list, generation_method: str, context: dict):
    """
    Finalize outfit by posting to Notion and cleaning up
    ...
    """
    try:
        # Step 1: Clear previous content from the page
        logging.info("Clearing previous content from Notion page...")
        clear_page_content(page_id)
        
        # Step 2: Post new outfit to Notion
        logging.info("Posting outfit to Notion...")
        post_outfit_to_notion_page(page_id, outfit_items)
        
        # Step 3: Clear trigger fields to reset for next use
        logging.info("Clearing trigger fields...")
...
def test_llm_agents(aesthetic: str = "Casual", user_prompt: str = "something comfortable for work"):
    """
    Test function to verify LLM agents are working
    ...
    try:
        ...
        llm_context = wardrobe_data_manager.get_llm_optimized_context(aesthetic, weather_tag)
        llm_context["user_prompt"] = user_prompt
        ...
        # Test Gemini
        print("\n🤖 Testing Gemini...")
        success, outfit, error = outfit_llm_agents.generate_outfit_with_gemini(llm_context)
        if success:
            print(f"✅ Gemini success: {[item['item'] for item in outfit]}")
        else:
            print(f"❌ Gemini failed: {error}")
        ...
        # Test Groq
        print("\n🤖 Testing Groq...")
        success, outfit, error = outfit_llm_agents.generate_outfit_with_groq(llm_context)
        if success:
            print(f"✅ Groq success: {[item['item'] for item in outfit]}")
        else:
            print(f"❌ Groq failed: {error}")
...
def run_outfit_pipeline(trigger_data):
    ...
    logging.warning("Using legacy run_outfit_pipeline - consider updating to run_enhanced_outfit_pipeline")
    return run_enhanced_outfit_pipeline(trigger_data)